...
async function getByProductId(product_id) {	  
	try {   
			var sql='select * from '+table+' where blog_id ='+product_id;
			console.log(sql);    
			return new Promise((resolve,reject)=>{
				connectPool.query(sql, (err, result) => {
					if (err) { 
...
async function setDefaultImage(id, product_id) {	  
	try { 
		if(id){ 
			...
			var sql1='UPDATE '+table+' set default_image = 0 where blog_id = '+ product_id;  
...
async function getDefaultImage(product_id) {	  
	try {   
		var sql='select * from '+table+' where blog_id ='+product_id+' AND  default_image = 1'; 
			return new Promise((resolve,reject)=>{
				connectPool.query(sql, (err, result) => {
					if (err) { 
						reject(err)
...
module.exports={
	...
	{	
        var BlogImages = sequelize.define('blog_images', {
              id: {
                type: type.INTEGER,
                primaryKey: true,
                autoIncrement: true
